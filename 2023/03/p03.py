...
for i, line in enumerate(engine):
    ...
    # Part 2
    possible_gears_iter = re.finditer(r"\*", line)
    for gear_match in possible_gears_iter:
        ...
        if i != 0:
            if engine[i - 1][index].isdigit():
                up_matches = re.finditer(r"\d+", engine[i - 1][index - 2 : index + 3])
                for up_match in up_matches:
                    if 2 >= up_match.span()[0] and 3 <= up_match.span()[1]:
                        up = [up_match.group()]
            else:
                up = re.findall(r"\d+$", engine[i - 1][left_index:index]) + re.findall(
                    r"^\d+", engine[i - 1][index + 1 : right_index + 1]
                )
        if i != 139:
            if engine[i + 1][index].isdigit():
                down_matches = re.finditer(r"\d+", engine[i + 1][index - 2 : index + 3])
                for down_match in down_matches:
                    if 2 >= down_match.span()[0] and 3 <= down_match.span()[1]:
                        down = [down_match.group()]
            else:
                down = re.findall(
                    r"\d+$", engine[i + 1][left_index:index]
                ) + re.findall(r"^\d+", engine[i + 1][index + 1 : right_index + 1])
        left = re.findall(r"\d+$", line[left_index:index])
        right = re.findall(r"^\d+", line[index + 1 : right_index + 1])
